<script lang="ts">
	...
	if (data.project) {
		data.form.data.id = data.project.id
		data.form.data.name = data.project.name
		data.form.data.clientCode = data.project.clientCode
		data.form.data.clientName = data.project.clientName
		data.form.data.status = data.project.status
	}
...
<form method="POST" enctype="multipart/form-data" autocomplete="off" use:enhance>
	...
	<Form.Field form={formAction} name="image">
		...
		<Form.Description>
			Leave this field empty if you don't want to change Project image preview.
		</Form.Description>
		<Form.FieldErrors />
	</Form.Field>
